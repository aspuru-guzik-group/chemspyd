...
class ChemspeedController(object):
    ...
    def transfer_liquid(
        ...
        if not validate.validate_zones(
            self.valid_zones, source
        ) or not validate.validate_zones(self.valid_zones, destination):
    ...
    def set_stir(self, stir_zone: str, state: str, rpm: float = 0):
        ...
        assert (
            stir_zone == "ISYNTH" or stir_zone == "RACK_HS"
        ), f"{stir_zone} zone not stirrable."
        assert (stir_zone == "RACK_HS" and rpm <= 400) or (
            stir_zone == "ISYNTH" and rpm <= 1600
        ), f"RPM out of range for {stir_zone}."
        self.unmount_all()
